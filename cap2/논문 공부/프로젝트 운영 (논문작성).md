...
```
[샘플 준비]
   ↓ (PVA 농도/동결-해동 시편 제작, 마킹)

[초음파 취득 (Verasonics 64E, L7-4/L22-14vX)]
   ↓ (RF/IQ, B-mode, timetag 저장)

[라벨 취득 (UTM, ASTM 규격 시험)]
   ↓ (Young’s modulus, 강도 등 확보)
...
[QUS 피처 추출]
   ...
   - Effective Scatterer Diameter (ESD)
   - Effective Acoustic Concentration (EAC)
   - Nakagami, 스펙트럼 기울기, SLSC 등
   ↓

[AI 모델 학습]
   - Branch A: BGN-B-mode CNN
   - Branch B: QUS 피처 CNN/MLP
   - Fusion → Regression Head
   - 출력: Young’s modulus + 보조 타깃
   ↓

[평가/검증]
   - Leave-concentration-out
   - Leave-probe-out
   - 재현성 (CoV)
   - 지표: MAE, RMSE, R², Spearman ρ
   - Bland-Altman 분석
   ↓
...
3. BGN 정규화:  
    E~(x,z)=E(x,z)/(ϵ+N(z))\tilde{E}(x,z) = E(x,z) / (\epsilon + N(z))
...
- ESD/EAC (Gaussian form factor 기반)
    
- Nakagami m, 스펙트럼 기울기, 엔트로피, SLSC 등
